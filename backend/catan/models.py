...
class Player(models.Model):
    ...
    development_cards = models.IntegerField(default=0, validators=[MinValueValidator(0)])
    resources_cards = models.IntegerField(default=0, validators=[MinValueValidator(0)])
...
class Card(models.Model):
    ...
    name = models.CharField(max_length=50, choices=CARD_TYPE)
...
class Vertex_position_settlements(models.Model):
    player = models.ForeignKey(Player, related_name='settlements', on_delete=models.CASCADE)
...
class Vertex_position_cities(models.Model):
    player = models.ForeignKey(Player, related_name='cities', on_delete=models.CASCADE)
    level = models.IntegerField(default=0, validators=[MinValueValidator(0),
                                                        MaxValueValidator(2)])
    index = models.IntegerField(default=0, validators=[MinValueValidator(0),
                                                        MaxValueValidator(29)])
...
class Road_position(models.Model):
    player = models.ForeignKey(Player, related_name='roads', on_delete=models.CASCADE)
    ...
    def __str__ (self):
        return '((%d, %d),(%d, %d))' % (self.level1, self.index1, self.level2, self.index2)
...
class Last_gained(models.Model):
    player = models.ForeignKey(Player, related_name='last_gained', on_delete=models.CASCADE, null=True)
    resources = models.ForeignKey(Resource, on_delete=models.CASCADE, null=True, blank=True)
